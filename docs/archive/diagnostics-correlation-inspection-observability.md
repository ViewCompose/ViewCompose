...
Complete. Phases 0 through 6 were accepted on 2026-08-24 and this plan is retained as historical
execution evidence.
This plan was split out on 2026-08-18 from the remaining diagnostics candidates in the unified
roadmap, the Diagnostics guide, and Phase 3 of the Performance specification. During execution it
was the sole owner of cross-session diagnostic correlation, production failure aggregation, real
View-boundary highlighting, and per-node timing; current conclusions now live in the owning active
documents.
...
Next action: none within this plan. Any further diagnostics capability or continuous observation
requires a new attributed plan and, where it changes inactive ownership, a new ADR. Maintained
performance and release conclusions now live in the Diagnostics guide, Performance specification,
and unified roadmap.
...
- `release/changes/20260824-correlated-device-diagnostics-inspector.json`
- `release/changes/20260824-diagnostics-phase6-verification.json`

The Phase 6 entry classifies `build.gradle.kts` as a shared no-release change. The phase changes only
the non-published benchmark harness, verification tasks, tests, and durable documentation, so no
published artifact receives a version increment.
...
| --- | --- | --- |
| [Unified roadmap](../project/roadmap.md), Runtime Effects / Transactions | Listed production diagnostic aggregation and exception sampling as a future focus | Superseded for execution tracking. The roadmap retains a summary and links to this plan. |
| [Unified roadmap](../project/roadmap.md), Diagnostics and Milestone D | Listed node highlighting, cross-session correlation, per-node timing, and a panel for high-frequency problems | Superseded for execution tracking. This plan owns the remaining diagnostics scope; baseline-profile work remains in the performance roadmap. |
...
| 2026-08-24 | Phase 5 same-worktree acceptance: 524 affected Foundation, Host, Preview, Runner, and Demo tests passed; the Studio plugin ran 129 tests with 127 passed and 2 skipped, built successfully, and verified compatible with all three configured Android Studio 2026.1 builds. `qaPreview` passed 1,115 tasks with 134 executed and 981 up-to-date; `qaQuick` passed 1,672 tasks with 275 executed and 1,397 up-to-date. Documentation, translation, release-intent, API-sample, and development-tooling-isolation gates passed. On one rooted Xiaomi MI 6 / Android 9, protocol v7 exposed one Host and three visible LazyItem Sessions with the correct parent graph, safe frame summaries, no retained failures, and no message, cause, stack, key, node key, text, content, or native View data. A request returned nine mounted nodes; the AndroidView target exercised fully clipped fail-closed behavior, selected exact screen bounds `(72,1320)-(1008,1584)`, and cleared. In the independently launched Studio sandbox, manual inspection covered refresh, Session selection, all three tabs, node loading, highlight/clear, stable automation roles, and source navigation. The inspection found that an internal `LocalValue.kt:169` label navigated to authored code; the same phase hard-cut Session, node, and timing rows to display the exact resolved project location used by navigation, then verified `DemoDiagnosticsPage.kt:189` both in the list and editor. The finite-timing tab remained visibly uncollected, proving that opening the inspector did not start timing. | Functional troubleshooting is **improved**. Four separate source/highlight/clear/timing actions and protocol v6 are replaced by one protocol-v7 inspector that correlates the Session parent tree, latest committed and attempted frames, latest privacy-safe failure, source, current mounted nodes, highlight state, and explicitly requested timing. Request-only weak inspection ownership adds no listener, polling loop, history, or second diagnostics bus; stale protocol reports fail closed. Manual verification also converted a misleading presentation/navigation split into one resolved-location contract across every navigable row. | Comparison is the Phase 4 tree with four independent actions, no correlated Studio surface, and no current diagnostic snapshot in the report. The result establishes improved workflow and correctness, not rendering performance. The one-device Android 9 run and one sandboxed Studio host do not form a compatibility or power matrix; the selected overlay is substantiated by exact protocol bounds and state while screenshot contrast is not a calibrated geometry measurement. Idle P50/P95, explicit request cost, power, leak/lifecycle, release APK classpath, Maven consumer, and broader device evidence remain **inconclusive** and are owned by Phase 6. Begin Phase 6 closeout. |
| 2026-08-24 | Phase 6 used the rooted Xiaomi MI 6 / Android 9 at fixed CPU 1.4016/1.8048 GHz, GPU 515 MHz, and `cpubw`/`gpubw` 13763. The same Debug APK workload and benchmark harness compared Phase 4 commit `da67ad78` with the Phase 6 candidate over five 16-fling iterations. Baseline/candidate frame CPU P50 was 2.684531/2.769011 ms and P95 was 5.012443/5.200990 ms; run-P50 CV was 0.0155/0.0153. Both APKs produced exactly zero v6/v7 report writes while idle. Twenty separately requested protocol-v7 source refreshes returned a fixed 32,633-byte, two-Session report with host end-to-end P50/P95/max 161.304/175.494/175.936 ms at 34.0 °C start/end. Forty-one lifecycle/failure tests, sixteen device-tooling tests, and three measurement-tool tests passed. The optimized Release APK passed a byte-level forbidden-marker scan after its release runtime graph excluded every tooling module; isolated Maven consumers built against the locally published repository. Combined `qaQuick` and `qaPreview` passed 1,947 tasks with 179 executed and 1,768 up-to-date. Final `qaFull` passed 2,086 tasks with 167 executed and 1,919 up-to-date; Xiaomi device suites passed Counter 1/1, Demo 139/139, and Tutorials 2/2 with no skip or failure. | Idle performance is **no material change**. P50 increased 0.084480 ms (3.15%) and P95 increased 0.188547 ms (3.76%); neither result crosses both its relative and absolute failure thresholds. Request cost is bounded well below ADR-0021's two-second request budget and is never amortized into idle results. Lifecycle ownership is mechanically weak, terminal after disposal, one-time registered, and request/report counts remain exact. Release isolation and Maven consumption pass. | Comparison is same-device, same fixed-clock, same Debug workload and harness; AndroidX's `DEBUGGABLE` warning is deliberately suppressed because the question is optional Debug-tooling cost, so these values are not Release performance. Host latency includes adb broadcast and polling and is not in-process serialization time. One Android 9 phone, one viewport, and no calibrated power rail do not establish a compatibility or power matrix; fixed clocks and unchanged 34.0 °C bound frequency and thermal variance only. No Phase 6 production or publication input changed; the Phase 6 Changeset records only the required shared no-release classification for the root verification task. Future expansion requires a new attributed plan. |
...
13. 2026-08-24 — Complete Phase 5 by hard-replacing the four independent Studio actions with one
    protocol-v7 correlated inspector. Add one request-only, weak-owned diagnostic snapshot to the
    existing inspection port; keep Session/frame/failure, mounted-node/highlight, and finite timing
    in one surface with stable automation roles. Reject old reports, resolve every navigable row to
    the exact project location it displays, and preserve zero polling, listener, recurring capture,
    or second diagnostics bus. Move all performance and release conclusions to Phase 6.
