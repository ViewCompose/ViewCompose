...
| ViewModel owner propagation | Lifecycle 2.11 creates arbitrary retained child UI scopes with `ViewModelStoreProvider`, inherits parent factories and `CreationExtras`, and protects exit-animation users with reference tokens. Compose-owned roots discover ViewTree owners. | Activity roots discover their `ViewTreeViewModelStoreOwner`; Fragment hosts explicitly retain the Fragment owner instead of the shorter View owner; nested explicit providers win. `ViewModelScopeProvider` supplies arbitrary subtree, navigation-entry, and navigation-graph stores over the same AndroidX primitive with stable identity, terminal clear, and no resurrection. Unlike Compose's positional convenience, ViewCompose always requires a caller-owned provider key. | Intentionally different | [`AndroidHostBridge.kt`](../../viewcompose-android/src/main/java/com/viewcompose/android/AndroidHostBridge.kt), [`ViewModelScopeProvider.kt`](../../viewcompose-viewmodel-androidx/src/main/java/com/viewcompose/viewmodel/ViewModelScopeProvider.kt), [`FragmentHostLifecycleIntegrationTest.kt`](../../viewcompose-android/src/test/java/com/viewcompose/android/FragmentHostLifecycleIntegrationTest.kt), and navigation recreation coverage in [`NavEntryOwnerStoreTest.kt`](../../viewcompose-navigation-android/src/test/java/com/viewcompose/navigation/NavEntryOwnerStoreTest.kt). |
| Saved state | Compose host integrations combine `SavedStateRegistryOwner`, `SavedStateHandle`, and saveable-state facilities. | ViewCompose hosts install a ViewCompose `SaveableStateRegistry`; Activity, Fragment, destination, and graph owners also support AndroidX saved state. UI-only values use `rememberSaveable`; restored business values have one ViewModel-owned `SavedStateHandle` flow. | Intentionally different | [`SavedStateViewModelIntegrationTest.kt`](../../viewcompose-viewmodel-androidx/src/test/java/com/viewcompose/viewmodel/SavedStateViewModelIntegrationTest.kt) and saved-state coverage in [`NavHostPublicApiTest.kt`](../../viewcompose-navigation-android/src/test/java/com/viewcompose/navigation/NavHostPublicApiTest.kt). |
...
equal child keys share only within that provider. Navigation entry and graph owners now use this
same provider: a saved host-scope identity retains their stores across configuration recreation,
while pop, graph removal, and normal host removal are terminal signals.

The aggregate host resolves ownership only at the boundary that owns the Android root. Activity
content discovers the root's `ViewTreeViewModelStoreOwner`. Fragment content deliberately uses the
Fragment as its explicit store owner even though the root ViewTree exposes the shorter-lived
`FragmentViewLifecycleOwner`; this keeps Fragment-scoped models across View recreation. A nested
`ProvideViewModelStoreOwner` still wins for its subtree. Low-level `renderInto` remains owner-free
and performs no ViewTree discovery.
...
The standalone `savedStateHandle()` function and `SavedStateHandleHolderViewModel` have been
removed without aliases. Move the old helper key to the real business ViewModel key, inject the
handle through the owner's default Factory or create it inside the initializer, and move each
restored business value into that ViewModel. Do not retain a compatibility holder, dual-write a
value through `rememberSaveable` and `SavedStateHandle`, or add a snapshot adapter merely to copy
Compose API shape.
...
process recreation separately from in-memory configuration change. `rememberSaveable` remains the
owner for UI-only state; `SavedStateHandle.getMutableStateFlow()` remains the business-state path.
...
- `NavHost` now fails directly when no `LocalViewModelStoreOwner` is present; custom `renderInto`
  hosts must provide it explicitly.
...
The clean Phase 3 run passed 151/151 Navigation Android cases and 21/21 Android aggregate-host
cases. Compared with the 148-test navigation baseline, the added contracts cover missing-owner
failure, configuration-retained entry ViewModels, prior-format restoration, Activity ViewTree
discovery, and Fragment explicit-owner precedence. Conclusion: **improved** for executable owner
selection and retention. Device process-kill, memory, and performance behavior were not measured,
so those dimensions remain **inconclusive**.
