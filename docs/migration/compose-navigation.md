...
| Adaptive panes and overlays | Navigation 3 scenes can select one or more entries and coordinate overlays and transitions. Versions 1.1.3 and 1.1.4 fix nested-overlay and popped-entry metadata animation defects. | ViewCompose exposes a general platform-neutral semantic scene value and a fixed host policy for up to three newest pane entries. The Android host does not yet consume overlay and transition roles as one execution plan. | Partially supported | [`NavScene.kt`](../../viewcompose-navigation-core/src/main/kotlin/com/viewcompose/navigation/core/NavScene.kt), host runtime, and lifecycle planner tests. The Navigation3 fixes change the comparison baseline but do not prove equivalent host behavior. |
...
sets beside the scene. The Android host consumes settled pane projections in this phase; complete
transition-role integration is tracked separately and must not be inferred from the Core model tests.
