...
`viewcompose-overlay-android` is the optional Android presentation backend for ViewCompose dialogs,
anchored popups, modal bottom sheets, snackbars, and toasts. The declarative overlay protocol, DSL,
queue policy, positioning model, and nested surface sessions live in `viewcompose-widget-core`; this
...
The artifact registers `AndroidOverlayHostFactoryProvider` through Java `ServiceLoader`. Normal
ViewCompose hosts call `OverlayHostDefaults.androidOrNoOp(rootView)`, so adding the dependency is
...
Widget core owns the shared snackbar/toast queue; this module supplies only platform presenters.
...
Most application tests should exercise declarative requests through the widget-core recording hosts
...
Custom platform backends can implement the widget-core presenter and handle interfaces without
