...
subsequent prerelease review exposed four additional confirmed defects, `MR-012` through `MR-015`;
the maintainer authorized that follow-up batch on 2026-08-18, and all four repairs now pass focused
automated and SM-G991B / Android 13 verification. The wider bilingual, theme, layout-direction,
enlarged-font, and representative-density matrix remains part of final plan closure.
...
`viewcompose-overlay-android`; the Changeset above owns those published-artifact corrections. No
...
## Manual-review defect repair backlog

This backlog distinguishes a Demo fixture defect from framework behavior exposed by the Demo.
The initial entries were frozen while implementation was deferred, then retained as repair and
acceptance records after the maintainer authorized that batch on 2026-08-17. The subsequent
prerelease findings were authorized on 2026-08-18 and are retained under the same rule. Each status
reports the current outcome without erasing the original diagnosis, an invalidated intermediate
closure, or the required wider closure matrix.

A reported rapid `Hide Tooltip` then `Show Tooltip` observation was withdrawn after reproduction
showed two consecutive user actions rather than a persistent visibility defect. It has no backlog
ID and no Tooltip behavior change is included in this repair batch.

<div className="search-partition-detail">

| ID | Scenario and observation | Classification | Evidence and implementation acceptance |
| --- | --- | --- | --- |
| `MR-001` | `layout.flow`: the third `FlowColumn` column compresses `Vertical item 7` through `Vertical item 9` into a few characters per line. | Confirmed framework defect in `viewcompose-renderer-android`; Demo usage is valid. Status: fixed; focused verification passed. | The fixture gives `FlowColumn` the available width, a fixed height, nine wrap-content children, and `maxItemsInEachColumn = 3`; it does not assign a narrower width to later columns. `DeclarativeFlowColumnLayout.onMeasure` instead passes the accumulated completed-column width as `widthUsed` to `measureChildWithMargins`, so each later column is measured against progressively less width. This contradicts the column-wrapping contract and the layout pass then preserves the already-compressed measurements. The repair must keep later columns from inheriting consumed cross-axis width, add a focused exact-width three-column regression that preserves equal intrinsic child widths, cover LTR and RTL placement, explicitly test behavior when natural column widths exceed the container, audit the symmetric `FlowRow` cross-axis measurement path, and pass a visual recheck of `layout.flow` in English and Simplified Chinese. |
| `MR-002` | `DemoEnvironmentActivity` reports the current locale but provides no application-language control. | Confirmed Demo defect; the framework resource environment is not at fault. Status: fixed; focused verification passed. | The active plan requires supported global theme, locale, direction, font-scale, and density controls to live in the Environment panel, but the implementation exposes only the global theme selector. The linked `environment.resources` fixture cannot substitute for this entry because its language action mutates only a scenario-owned `MutableContextWrapper` and intentionally leaves the application locale unchanged. The repair must add an explicit application-language selector for system default, English, and Simplified Chinese; apply and persist the per-app locale through the supported Android/AppCompat path; keep the isolated resource-propagation fixture independent; localize every label and state; verify that the Environment panel, catalog, and a separately launched scenario resolve the selected language; verify that clearing the override returns to the system locale; restore the original locale in automated tests; and document the distinction between global application language and scenario-local resource mutation in the on-screen scope note. |
| `MR-003` | Fixed-height multiline fields lose supporting content: `input.stress` vertically clips `Toggle read-only mode and expanded copy to verify multiline stability.`, while `input.fields` completely hides `Supports multiline editing and local state updates` below the `Summary` Bio editor. | Confirmed Demo layout defect; `TextField` and the Android renderer honor the constraints supplied by the fixtures. Status: fixed; focused verification passed. | The public `TextField` composite applies its modifier to the outer `Column` containing the label, editable node, and supporting text. Both fixtures configure `MultiLine(maxLines = 6)` but also fix that entire composite to `132.dp` or `120.dp`; after the label and multiline editor are measured and placed, the final supporting `Text` lies partly or wholly outside the fixed outer boundary. A 2026-08-17 device reproduction confirmed that the Bio label, editor, and supporting node all exist in the native hierarchy, but the supporting node is visually clipped, so this is not a missing VNode or lazy-item mount. Existing visual device tests assert only scenario actions, benchmark targets, the protected field, and reset behavior; neither supporting line has a stable selector or visible-bounds assertion. The repair must express intended editor height through appropriate multiline `minLines`/`maxLines` policies or another content-safe constraint instead of fixed outer-composite heights, audit every multiline `TextField` call site for the same misuse, add stable selectors plus full-visibility/non-ellipsized assertions for both supporting lines, and visually verify compact/expanded plus read-only/editable states in English and Simplified Chinese at default and enlarged font scales. |
| `MR-004` | `input.search`: the `188.dp`-high `ScrollableColumn` inside the outer `LazyColumn` leaves `Scrollable placeholder row 4` cut by the viewport edge and appears to fight the page scroll. | Confirmed Demo fixture defect; the renderer's same-axis edge handoff works as designed. Status: fixed; focused verification passed. | The fixed viewport is only about `12.dp` shorter than its padded note, `SearchBar`, four placeholder rows, and five spacing gaps, so the final row starts at the clipping boundary and the inner container reaches its end almost immediately. On a 2026-08-17 SM-G991B reproduction, a sub-edge drag changed only the inner content while the outer `RecyclerView` anchor stayed fixed; after the inner end was reached, the final row was fully inside the `ScrollView` bounds and only the remaining drag moved the outer page. This matches `DeclarativeScrollableColumnLayout`, which disallows parent interception while it can scroll in the gesture direction and releases it at the matching edge, as well as its focused unit test. The existing device test exercises focus reveal for the inner `SearchBar` but never asserts placeholder-row visibility or parent/child gesture ownership. Resolve this symptom through the isolated-fixture split required by `MR-005`, not by tuning the magic height inside the mixed page: the standalone fixture must use a locale- and font-scale-safe viewport/content arrangement whose initial boundary falls between complete children while preserving meaningful inner overflow, ensure every placeholder row can be fully revealed by inner scrolling, assert that a sub-edge drag leaves unrelated host chrome fixed before edge handoff, cover upward and downward handoff, and visually verify English and Simplified Chinese at default and enlarged font scales. |
| `MR-005` | `input.search` combines SearchBar behavior with four focus-follow paths in one outer `LazyColumn`, including nested `ScrollableColumn`, `VerticalPager`, and `PullToRefresh`; the resulting page cannot isolate which container moved or whether the intended focus-follow behavior worked. | Confirmed Demo verification-design defect; no framework defect is established by the current fixture or tests. Status: fixed; focused verification passed. | The route owns seven text states plus pager and refresh state, and mounts basic, clearable, and disabled SearchBars before three independent nested-container probes. All four focus tests launch the same route and reuse `assertFocusActionRevealsInput`; that helper checks only that the focused editor ends visible and that the first outer `RecyclerView` item's adapter position is unchanged. Because the outer page contains one oversized `search` item, its position remains `0` despite arbitrary pixel scrolling, while the pager editor starts visible inside page 0 and does not force the pager itself to demonstrate focus following. Replace the mixed route with five direct scenarios: retain `input.search` only for SearchBar submit/clear/disabled behavior, and add `input.focus-follow-lazy-column`, `input.focus-follow-scrollable-column`, `input.focus-follow-vertical-pager`, and `input.focus-follow-pull-refresh`. Each focus fixture must have non-scrolling host chrome and exactly one vertical scroll owner under test, except for the required `PullToRefresh` wrapper whose child remains the declared scroll owner; establish a deterministic pre-focus state in which showing the IME requires that owner to reveal the editor; expose stable root, ready, target, state, focus-action, and reset roles; assert the owning container's identity and before/after scroll or pager state, minimal reveal, full editor visibility, stable controlled page where applicable, and absence of unrelated host movement; retain direct manual gestures for both directions and edges; and cover English and Simplified Chinese at default and enlarged font scales. If an isolated fixture then fails, classify that independently against the responsible framework container rather than masking it with another outer scroller. |
| `MR-006` | `animation.specs`: the `animateContentSize` Surface is the final block in the page, so expanding it grows below the viewport and presents no observable size animation. | Confirmed Demo verification-design defect; frame evidence shows that the Android animated-size host does animate. Status: fixed; focused verification passed. | The block follows every typed-value, easing, repeat, reset, vector, and state control in the route's single oversized lazy item. At maximum scroll its collapsed bottom is already aligned with the screen edge, so the animated lower boundary and most revealed content leave the viewport. The block also hard-codes `spring()` and is unaffected by the page's selectable Tween, Spring, Keyframes, Snap, and Repeatable spec, making it an unrelated compatibility probe rather than part of the primary specs result. A 2026-08-17 SM-G991B recording with all system animation scales at `1.0` showed the expanded text nodes being removed on collapse and the Surface height then decreasing across successive frames during the declared `550 ms` spring; this rules out a framework no-animation defect. The existing device test clicks the size toggle only alongside three other actions and passes after the final probe Text is visible, without sampling host height or asserting that the fixture stays on screen. Extract this probe into a direct `animation.content-size` scenario whose non-scrolling stage keeps both animated boundaries visible with reserved expansion room; provide deterministic expand, collapse, reset, state, target, and endpoint anchors; keep `animation.specs` focused on the selected specification's typed and generic values; assert collapsed, at least one strictly intermediate, and expanded host height in both directions while unrelated host geometry remains fixed; verify clipping/reveal behavior and rapid in-flight retargeting; add focused runtime-host regression coverage for first measurement, growth, shrink, and retargeting without changing framework behavior unless the isolated fixture exposes a separate defect; and visually verify English and Simplified Chinese at default and enlarged font scales. |
| `MR-007` | `collection.lazy-list` and `collection.stress`: each mounts a fixed-height keyed `LazyColumn` inside the page's outer vertical `LazyColumn`; dragging inside the keyed list scrolls the page instead, and once the page reaches its end the keyed list cannot be scrolled to its own end. | Confirmed same-axis nested-list gesture defect in `viewcompose-renderer-android`; fixed-height nested collections are permitted by the DSL and nested-prefetch contract, while both Demo fixtures obscure the contract behind unrelated page scrolling. Status: fixed; focused verification passed. | `collection.lazy-list` gives the outer list separate `controls` and `list` items, then mounts its keyed list at `280.dp` with a sticky header, three stateful rows, and `LazyLayoutPrefetchPolicy.nestedInitialPrefetchItemCount = 4`. `collection.stress` uses the same outer `LazyColumn`, puts all controls and a `320.dp` keyed stress list inside its single oversized `stress` item, and therefore reproduces the same gesture conflict shown by manual review. The public contracts allow a lazy collection in `ScrollableScope` and define a nested-collection prefetch hint without documenting a same-axis exclusion. `DeclarativeLazyListView`, however, only gates `onInterceptTouchEvent` and `onTouchEvent` with `userScrollEnabled`; it has no direction-aware gesture reservation or edge handoff. On a 2026-08-17 SM-G991B reproduction of `collection.lazy-list`, an upward drag started inside the child moved the child viewport from top `1584` to `1332` solely because the outer page moved `252 px`; the sticky header and rows kept identical child-relative coordinates, and the child snapshot remained `Visible [0, 1, 2, 3] · can scroll forward true`. At the outer end, repeated child drags changed neither list. Existing lazy-list coverage toggles labels and checks the final visible A row, while the stress tests verify only controls, insertion, and reordered ID text; none drags a child list, verifies sticky pinning, observes child offsets, or proves edge handoff. The framework repair must give RecyclerView-backed lazy containers direction-aware ownership: a child retains an axis-matching gesture while it can scroll in that direction, releases only at the true edge, does not capture cross-axis gestures, and handles reverse layout, RTL horizontal lists, fling, `ACTION_CANCEL`, disabled scrolling, and rapid direction reversal; audit `LazyColumn`, `LazyRow`, and the corresponding grid/pager RecyclerView hosts for the same gap; add focused unit and real nested-RecyclerView device tests asserting child movement with a stable parent, then parent continuation at both child edges. The Demo repair must make both `collection.lazy-list` and `collection.stress` direct single-scroll-owner fixtures, with controls outside the scrolling viewport or represented as headers owned by the same collection; preserve and independently assert lazy-list keys, content revisions/types, sticky header, layout snapshots, reorder, and remembered row state, plus stress insertion, reorder, label mutation, item identity, and repeated-update behavior. Move same-axis handoff and nested-prefetch coverage to a direct `collection.nested-lazy-list` fixture with explicit parent/child state and targets instead of hiding it in either ordinary fixture; cover both locales and enlarged font scale. Because the `collection.stress` scroll and mutation workloads currently use accepted revision 2 coordinates and hierarchy, the fixture restructuring invalidates that revision: advance the workload revision, recapture baseline and comparison evidence, and update the owning active performance interpretation before accepting the change. A renderer production correction requires the owning-module behavior documentation and immutable release changeset in the implementation batch. |
| `MR-008` | `collection.grid`: the rounded grey grid surface ends with an inset rectangular strip and pointed teeth below both bottom corners. | Confirmed Demo modifier defect; the framework's explicit shape-versus-content-clipping contract behaves as designed. Status: fixed; focused verification passed. | The `LazyVerticalGrid` applies `fillMaxWidth`, fixed `400.dp` height, variant background, and `SurfaceDefaults.shape()`, but never applies `clip()`. Its six children are filled `Card` surfaces and the grid has `8.dp` content padding. The public modifier contract exposes `clip()` separately, and `ModifierSurfaceStyleApplier` intentionally enables `clipToOutline` only for an explicit clip request; `DeclarativeLazyVerticalGridLayout.dispatchDraw` otherwise clips descendants only to the rectangular View bounds. On a 2026-08-17 SM-G991B reproduction, the grid bounds were `[72,972][1008,2172]`; an enlarged bottom crop showed the last row's same-coloured child surfaces continuing to the rectangular bottom while the rounded parent background receded, with the protruding strip inset by approximately the declared `8.dp` padding. This rules out a malformed `UiShapeDrawable` curve and identifies missing content clipping in the fixture. Existing device coverage toggles two versus three columns and checks only the first label; it does not identify the grid host, inspect `clipToOutline`, reveal the bottom edge, or compare pixels outside the resolved shape. Repair the fixture by making the rounded surface own explicit descendant clipping, either by adding `clip()` to the shaped grid host or by using an equivalent shaped clipping container without changing global shape semantics. Add a stable grid target and focused assertions that the host is fully visible, the first and last rows remain reachable, no child pixels escape the resolved outline at any corner, and switching between two and three columns preserves clipping and content; visually verify both themes, both locales, LTR and RTL, and default and enlarged font scales. No renderer behavior change or release changeset is warranted unless the explicit-clip version still leaks in an isolated renderer test. |
| `MR-009` | `collection.pull-refresh`: a real downward gesture on the `ScrollableColumn` neither reveals the pull indicator nor invokes refresh, so the route can show loading only through its simulation button. | Confirmed framework gesture-integration defect between the Android `PullToRefresh` host and its supported eager scroll child; the Demo's outer same-axis `LazyColumn` is an additional verification-design defect. Status: fixed; focused verification passed. | The route mounts a fixed `300.dp` `PullToRefresh` around a `ScrollableColumn`, but places that pair below state and action controls inside the sole oversized item of the page's outer `LazyColumn`. `PullToRefresh` publicly accepts `ScrollableScope` content, its compiled Q3 sample uses `ScrollableColumn`, and the renderer manual states that eager vertical containers reserve a pointer stream only while they can consume its direction. The eager child nevertheless reserves ancestor interception on `ACTION_DOWN` whenever either scroll direction remains and releases it only after resolving movement; `DeclarativePullToRefreshLayout` adds no coordination beyond stock `SwipeRefreshLayout`. On a 2026-08-17 SM-G991B reproduction with the child at its top and the outer page stationary, both a `600 px / 1 s` pull and a `650 px / 5 s` pull left the counter at zero, retained `Simulate refresh`, and showed no indicator even in a screenshot captured during the live gesture. Clicking the simulation button immediately changed the state to `Refreshing…`, incremented the count, and displayed the native spinner, proving that state binding, callback-side recomposition, indicator colour, and loading drawing work while the touch path does not. The renderer tests invoke `OnRefreshListener` directly and cover enablement/listener reuse but never dispatch a pull through a real `ScrollableColumn`; the Demo registry exposes only root, ready, and reset, and no device test covers this route. The framework repair must make `PullToRefresh` and each supported vertical child (`ScrollableColumn`, `LazyColumn`, and `LazyVerticalGrid`) negotiate one downward stream correctly: child scrolling wins away from its logical top, the refresh host owns unconsumed downward drag at the top, threshold crossing invokes exactly one callback, sub-threshold/cancelled/multi-pointer gestures do not, refreshing and disabled states remain deterministic, and unused distance may reach an outer parent only after the refresh contract has resolved it. Add focused Android unit tests plus real-device gestures for direct and same-axis-nested hosts, both child edges, direction reversal, fling, `ACTION_CANCEL`, enabled/disabled, and active-refresh input suppression. Rebuild `collection.pull-refresh` as a direct single-scroll-owner fixture whose non-scrolling chrome leaves the refresh viewport fully visible and whose child has intentional overflow; expose stable state, target, primary pull action evidence, simulation fallback, completion, and reset roles; assert idle → pull → refreshing → completed transitions, spinner visibility during loading, exactly one count increment, child-scroll preservation, and repeatability. The simulation control remains an accessibility and state-binding path, not evidence that the pull gesture works. Cover both locales and enlarged font scale. The renderer correction requires owning-module behavior documentation and an immutable release changeset; the isolated pull fixture required by `MR-005` must use the corrected contract rather than adding another workaround. |
| `MR-010` | `overlay.menu`: the rounded dropdown card ends in a hard grey strip with dark wedges below its bottom corners instead of one shape-matched shadow. | Confirmed framework visual defect in `viewcompose-overlay-android`; the Demo uses the public `DropdownMenu` composite without overriding its surface or elevation. Status: fixed; focused verification passed. | `DropdownMenu` already gives its rounded menu surface the theme-owned `3.dp` native elevation, while every Android popup handle independently assigns `12.dp` elevation to the surrounding transparent rectangular `PopupWindow`. The popup is then measured to the exact content size, leaving no owned outset for the child surface shadow. On a 2026-08-17 SM-G991B reproduction, the menu window frame was exactly `[72,1201][408,1825]`, and the bottom artifact aligned with that rectangular window boundary rather than the menu's rounded outline. The result combines two elevation owners and clips or reshapes the inner rounded shadow at the platform-window edge; this is not caused by Demo modifiers. Existing Android overlay coverage verifies only that a popup is shown and dismissed and never asserts elevation ownership, window outsets, resolved outline, or pixels around the surface. The framework repair must establish one explicit shadow owner for popup surfaces: generic `Popup` must not impose an undocumented competing elevation, while `DropdownMenu`, `PlainTooltip`, and caller-defined popup content retain their declared visual contracts; any required shadow outsets must participate in measurement and placement without changing the content anchor alignment or overflow policy. Add focused presenter/rendering tests for rounded and rectangular content, zero and non-zero content elevation, all popup alignments, flip/clamp near every viewport edge, LTR/RTL, content resizing, and show/update/dismiss cycles. Add a stable menu-surface target and screenshot/pixel assertions that the complete rounded outline and one continuous shadow remain visible with no rectangular strip, clipped corner, or double shadow in both themes, both locales, and representative densities. The production correction requires the owning overlay-module behavior documentation and an immutable release changeset in the implementation batch. |
| `MR-011` | `component.fab`: the primary FAB's left shadow is cut off at the page content boundary; the leftmost size variant and both extended variants show the same truncation. | Confirmed Demo fixture layout defect; the framework FAB elevation and lazy-viewport clipping contracts behave as designed. Status: fixed; focused verification passed. | The fixture mounts a zero-content-padding `LazyColumn`; its header and variants place elevated FABs flush against the viewport's left edge. `FloatingActionButton` and `ExtendedFloatingActionButton` intentionally keep their semantic layout bounds equal to the declared control size and render native elevation outside those bounds. Lazy item decoration hosts allow drawing beyond an individual holder, but `DeclarativeLazyListView.dispatchDraw` deliberately clips final output to the scroll viewport. On a 2026-08-17 SM-G991B reproduction, the primary FAB bounds were `[24,548][192,716]` and its left shadow terminated exactly at `x=24`; every other left-aligned elevated variant began on the same boundary and reproduced the cut. This is required viewport clipping applied to content that the Demo failed to inset, not a malformed FAB outline or renderer shadow. Existing component automation verifies primary-action state and full-Session reset, while the smoke test has no stable per-variant shadow targets or pixel/outset assertions. Repair the fixture by reserving explicit horizontal and vertical shadow-safe space inside the lazy viewport for every elevated variant, using content padding or equivalent item insets derived from the largest supported elevation rather than weakening viewport clipping or inflating FAB touch bounds. Preserve the declared small, medium, large, and extended semantic sizes, row spacing, click targets, and reset behavior. Add stable targets and screenshot/pixel assertions that all four sides of each shadow remain continuous and inside the viewport, no neighbouring item overlaps an outset, and LTR/RTL place equivalent start/end clearance; visually verify the primary, all three size tiers, extended, and custom-colour variants in both themes, both locales, default and enlarged font scales, and representative densities. No framework source change or release changeset is warranted unless an isolated, correctly inset lazy fixture still clips inside the viewport. |
| `MR-012` | `overlay.menu`: after the `MR-010` repair, the dropdown still shows a flattened or clipped shadow along the card's bottom edge on the prerelease build. | Confirmed framework visual defect in `viewcompose-overlay-android`; this is incomplete shadow-outset handling left by the first repair, not a Demo modifier defect. Status: fixed; focused unit and reference-device screenshot verification passed. | The first repair correctly removed the competing `PopupWindow` elevation and made the popup container transparent, but `AndroidPopupOverlayHandle` still measures and sizes the window to the exact content bounds. `DropdownMenu` continues to render its declared `3.dp` Surface elevation outside those bounds, so the popup window clips the native shadow at its own rectangular edge; the new photo is consistent with that remaining geometry. The next repair must give elevated popup content an explicit visual outset, or an equivalent internal content-versus-window bounds contract, while keeping anchor calculations based on semantic content bounds. It must preserve horizontal alignment, vertical flip and viewport clamp behavior, outside-touch dismissal, content resizing, LTR/RTL, and zero-elevation generic `Popup` behavior. Extend presenter and screenshot coverage to assert non-background pixels around every rounded corner and below the content boundary at representative densities, themes, and edge placements; a test that checks only popup visibility or an in-bounds screenshot is insufficient. Reopen the overlay-module documentation and release-change assessment when implementation is authorized. |
| `MR-013` | `foundations.theme` and the Material 3 theme-bundle pages compress three colour-token swatches into a narrow strip, forcing short labels such as `OnP` and Chinese token names to wrap or disappear. | Confirmed shared Demo layout defect; the Row weight implementation is not at fault. Status: fixed; focused measurement and device verification passed. | `ThemeSwatchRow` gives its inner swatch `Row` `fillMaxWidth()` and gives each swatch column `weight(1f)`, but the outer helper `Column` remains wrap-content. The Row therefore receives the narrow intrinsic constraint of its ancestor and divides that already-collapsed width among the weighted children. The same helper is reused across the affected theme pages, which explains the repeated symptom in the supplied light and dark bundle photos. The repair must make the shared swatch section own the available width and keep each colour chip and label legible without relying on one locale's intrinsic text width. Add focused layout and screenshot checks for two-, three-, and four-swatch groups, short and long labels, English and Simplified Chinese, default and enlarged font scales, both directions, and every theme-bundle source; assert complete labels and non-overlapping swatch bounds. Do not change framework weight semantics unless an isolated correctly constrained Row reproduces the defect. |
| `MR-014` | `NavigationBar`: tapping directly on an item's star icon or label sometimes performs the action without a visible ripple/state-layer response. OneUI text navigation reproduces the same symptom. | Confirmed framework interaction-feedback and drawable-lifecycle defect in `viewcompose-renderer-android`; hit testing reaches both targets. Status: fixed after a video-driven follow-up; quick-tap identity and visible-frame verification pass for the standard icon target and OneUI text target. | The first repair moved each standard NavigationBar ripple from the item background to its foreground, preventing the indicator, icon, badge, and label from obscuring it. A supplied 6.7-second recording nevertheless showed that a quick star tap still selected the item without a visible release animation, invalidating the earlier held-press closure. The click callback synchronously recomposes selection: `DeclarativeNavigationBarLayout` then replaced the foreground `RippleDrawable` when selected and unselected colors differed, while the generic Modifier surface path rebuilt the BasicSurface background used by OneUI text navigation for the same color-only change. Both replacements destroyed the drawable that owned the active release animation. The completed repair preserves drawable identity and updates its `ColorStateList` in place for color-only state-layer patches, while retaining full rebuilds for structural surface changes. Focused unit coverage asserts identity through selected-role patches; SM-G991B / Android 13 tests dispatch 16 ms down/up sequences at the standard star and OneUI label, capture the 48 ms release frame, assert selection, and assert the same ripple instance remains mounted. The pulled screenshots visibly contain the expanding standard item circle and OneUI text-item state layer. The renderer-module behavior documentation and existing immutable release changeset cover the corrected production behavior. |
| `MR-015` | The Material 3 Bundle's `SegmentedControl` selected segment has a corner curve that does not follow the outer border, so the inset fill looks detached from the container. | Confirmed Demo design-system recipe defect; the generic framework `SegmentedControl` contract is not implicated by the current evidence. Status: fixed; focused recipe-geometry and device verification passed. | `DemoDesignSegmentedControl` renders a `44.dp` outer container, applies `2.dp` inner padding, and renders a `40.dp` selected item. The Material 3 recipe uses an absolute `20.dp` rounded container shape but an independent `roundedRelative(0.5f)` item shape, which also resolves to `20.dp` at the smaller height. A concentric `2.dp` inset should resolve to an `18.dp` inner radius, so the two outlines cannot align. The repair must derive the item shape from the actual container shape and inset, rather than reuse the navigation-indicator shape, while retaining the intended cut and Cupertino bundle variants. Add geometry and screenshot assertions for first, middle, and last selections; two through five segments; rounded, cut, and continuous shapes; LTR/RTL; both themes; all bundle sources; and enlarged font scale. Confirm the generic `SegmentedControl` in an isolated fixture before deciding whether any framework change or release changeset is necessary. |

`MR-012` supersedes only the cross-device shadow-closure claim recorded for `MR-010`; the first
repair's removal of the competing popup-window elevation remains valid.
...
| Input | Fields, Selection, Stress, Search, Summary, 0-4 | Split; split Search again by vertical focus-follow owner after manual review | `input.fields`, `input.selection`, `input.stress`, `input.search`, `input.focus-follow-lazy-column`, `input.focus-follow-scrollable-column`, `input.focus-follow-vertical-pager`, `input.focus-follow-pull-refresh`, `input.derived-summary` |
| Feedback | Transient, Dialog, Menu, 0-2 | Split; retain overlay-capable host; repair popup shadow ownership after manual review | `overlay.transient`, `overlay.dialog`, `overlay.menu` |
| Collections | Controls, List, Stress, Interop, Row, Grid, Refresh, 0-6 | Split; give ordinary keyed-list, stress, and pull-refresh fixtures one scroll owner; isolate same-axis nested-list handoff and prefetch; repair explicit grid clipping and the real pull gesture after manual review | `collection.controls`, `collection.lazy-list`, `collection.nested-lazy-list`, `collection.stress`, `collection.android-view`, `collection.lazy-row`, `collection.grid`, `collection.pull-refresh` |
...
| Actions | Card, FAB, Chip, List item, 0-3 | Split; add shadow-safe FAB fixture insets after manual review | `component.card`, `component.fab`, `component.chip`, `component.list-item` |
...
| Animation | Core, Content, List motion, Specs, Transition, Infinite, 0-5 | Split; extract the size-animation compatibility probe after manual review | `animation.core`, `animation.content`, `animation.content-size`, `animation.list-motion`, `animation.specs`, `animation.transition`, `animation.infinite` |
...
Manual review on 2026-08-17 subsequently reproduced the same-axis gesture failure in both the keyed
list and collection-stress routes: an inner list reports or visibly retains remaining content while
its drag moves only the outer page. `MR-007` therefore reopens the renderer behavior and these two
Demo routes, and defers the direct `collection.nested-lazy-list` fixture recorded in the frozen
inventory. The seven-fixture and passing-test statements above remain evidence for the current
implementation, whose tests do not exercise nested child drag ownership or edge handoff. The
required stress-fixture hierarchy change also invalidates the accepted revision 2 scroll and
mutation workload evidence; its successor cannot be accepted without a revision bump and new
baseline interpretation.

The same manual review exposed a separate visual defect at the bottom of `collection.grid`. Device
pixel evidence ties the pointed strip to filled grid children escaping the rounded background where
the fixture omitted the framework's explicit clipping modifier. `MR-008` reopens only this Demo
route and its visual coverage; it does not reopen the renderer shape algorithm or its accepted
pixel evidence unless the corrected explicit-clip fixture still reproduces the leak.

Manual review of `collection.pull-refresh` then separated its loading presentation from its touch
path. The simulation action displays the native spinner and publishes refreshing state, while long
real pulls at the child top produce neither intermediate indicator movement nor a callback even
when the outer page remains stationary. `MR-009` therefore reopens the renderer interaction between
`PullToRefresh` and supported vertical children as well as this Demo route; simulation evidence is
not accepted as pull-gesture evidence.
...
Manual review on 2026-08-17 subsequently found that the strict `input.search` route still combines
four independently assertable focus-follow owners inside one outer scroller. `MR-005` therefore
reopens only that route and defers its replacement with the four direct `input.focus-follow-*`
fixtures recorded in the frozen inventory; the five-fixture statement above remains the current
implemented state until the correction batch is authorized.
...
tests passed. Manual review on 2026-08-17 subsequently found that the final
`animateContentSize` compatibility block is outside the observable area of the specs fixture and
is not driven by its selectable specification. `MR-006` therefore defers extraction of the direct
`animation.content-size` fixture recorded in the frozen inventory; the six-fixture statement and
passing tests above describe the current implementation and its insufficient final-state-only
coverage, not closure of the newly accepted defect.
...
Manual review on 2026-08-17 subsequently found that the dropdown surface's rounded `3.dp`
elevation is combined with the Android popup host's independent rectangular `12.dp` window
elevation. `MR-010` therefore reopens the Android popup presentation contract and the menu visual
coverage; the prior overlay flows did not inspect shadow ownership or pixels outside the content
bounds.
...
Manual review on 2026-08-17 subsequently found that every left-aligned elevated control in
`component.fab` is placed directly on the lazy viewport boundary, where the viewport correctly
clips its external native shadow. `MR-011` therefore reopens only the FAB fixture layout and its
visual coverage; the existing action/reset and component smoke tests do not inspect shadow outsets.
...
This repair changes published renderer and overlay production source. The branch records those
fixes in the immutable per-PR Changeset
`release/changes/20260817-demo-manual-review-fixes.json`; it leaves the merged
`release/changes/20260816-plain-text-binding-allocation.json` record unchanged.
...
| 2026-08-17 | Manual-review repair focused verification on SM-G991B / Android 13 plus JVM checks | All 11 confirmed defects are fixed. Renderer Flow and nested-gesture tests, overlay popup ownership tests, registry tests, AndroidTest compilation, benchmark compilation, documentation structure, and release intent passed. Six focused device tests passed: all four IME focus-follow owners revealed the complete editor above a visible keyboard, the rounded grid host clipped descendants, and a real downward pull started exactly one refresh before completion stopped the indicator. Manual device inspection confirmed equal FlowColumn columns, complete multiline supporting text, observable content-size expansion, direct collection scrolling and nested edge handoff, clean grid/menu shadows, FAB shadow clearance, compact application-language controls, and persisted Simplified Chinese selection with restoration to system locale. Conclusion: improved. Limitations: the full bilingual/theme/RTL/enlarged-font screenshot matrix was not rerun, and collection-stress revision 3 performance evidence remains intentionally unaccepted. Next action: run those remaining visual matrices during final plan closure and rebaseline revision 3 only on the required stable reference device. |
| 2026-08-18 | Post-prerelease manual review with five supplied device photos plus source and layout trace | Accepted four additional defects as `MR-012` through `MR-015`: the popup repair still clips its child Surface shadow at exact window bounds, the shared Demo swatch helper constrains weighted children through a wrap-content parent, the NavigationBar ripple sits below opaque child layers, and the Material 3 Demo recipe resolves equal outer and inner radii despite a `2.dp` inset. No implementation was performed. Conclusion: the initial repair remains improved overall, but menu-shadow closure is regressed across the broader device matrix and three additional fixture/feedback gaps are confirmed. Limitations: static photos cannot capture the NavigationBar's pressed frame, so final acceptance requires a real touch sequence and intermediate-frame evidence. Next action: wait for maintainer authorization, then implement and run the frozen focused plus wider visual matrices. |
| 2026-08-18 | Follow-up manual-review repair verification on SM-G991B / Android 13 plus repository checks | All four follow-up defects are fixed. Elevated popup content now reserves a transparent visual outset while semantic anchor placement and outside-touch dismissal remain content-based; the shared theme-swatch section owns full available width; NavigationBar state layers draw in the item foreground across icon and label regions; and every Demo segmented recipe derives its selected-item shape from a `2.dp` inset of the container shape. Full overlay, renderer, and Demo unit suites passed, as did `qaQuick`. Four focused device methods passed for popup interaction, NavigationBar real-touch pressed state and selection, the rounded Material 3 pressure slice, and scoped-theme behavior. The captured popup screenshot was pulled from the reference device and visually inspected: the rounded card now has one continuous, unclipped shadow on all four sides. Conclusion: improved. Limitations: the broader bilingual/theme/RTL/enlarged-font/representative-density screenshot matrix and automated popup pixel-golden comparison remain outstanding; the focused swatch measurement currently covers the three-token group that reproduced the defect. Next action: run the remaining visual matrix during final plan closure and keep Phase 5 performance work gated on a clock-controllable device. |
| 2026-08-18 | `MR-014` quick-tap closure correction from the supplied recording and OneUI reproduction | The 6.7-second standard NavigationBar recording invalidated the earlier held-press result: selection changed, but quick release feedback disappeared. OneUI text navigation reproduced the same failure. Source tracing showed that synchronous selected-state patches replaced the active `RippleDrawable` in both the dedicated NavigationBar foreground and the generic BasicSurface background. The correction now updates color selectors in place. Focused JVM tests pass, and 2/2 SM-G991B / Android 13 quick-tap paths passed: a 16 ms standard star tap and OneUI label tap both changed selection, retained the identical ripple object, and showed the release state in screenshots captured 48 ms after `ACTION_UP`. Conclusion: improved and the intermediate `MR-014` closure is superseded. Limitations: this correction was visually sampled in light theme, LTR, and one density; the full theme/RTL/locale/density matrix remains outstanding. Next action: keep the identity regressions in the normal suite and include both targets in final visual-matrix closure. |
