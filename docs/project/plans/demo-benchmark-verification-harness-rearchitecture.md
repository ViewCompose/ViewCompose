...
The maintainer authorized the human-review repair batch on 2026-08-17. All confirmed items are now
fixed; focused automated checks and reference-device verification pass. The wider bilingual,
theme, layout-direction, and enlarged-font matrix remains part of final plan closure.
...
The entries were frozen while implementation was deferred, then retained as repair and acceptance
records after the maintainer authorized the batch on 2026-08-17. Each status now reports the
implemented outcome without erasing the original diagnosis or required wider closure matrix.
...
This repair changes published renderer and overlay production source, but the working branch
already owns the single per-PR Changeset
`release/changes/20260816-plain-text-binding-allocation.json` through the active observed-property
plan. The renderer and overlay impact is merged into that existing branch record; this plan does
not claim the same immutable file a second time.
