...
Active. Phases 0 through 3 are complete; sampled per-node timing is next.
...
Next action: execute Phase 4 as one finite requested sample. Add the capture controller,
composition/reconciliation/binding timing records, inclusive/exclusive semantics, caps,
truncation and overhead reporting, deterministic fixtures, and Studio/Demo presentation while
preserving exactly zero per-node clock reads when inactive.
