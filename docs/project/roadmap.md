...
| AI-verifiable development tooling | Public release `0.5.0` provides the version-bound Knowledge Bundle/`llms.txt`, one-command Agent onboarding, and versioned high-confidence project analysis. The tested `0.6.0` candidate expands the package to 14 CLI/MCP tools and seven Agent Skills with strict offline Figma Design IR import, deterministic Kotlin/PNG generation, and compile/Preview/bounded structure-semantic-geometry-asset comparison; it does not claim style or pixel parity | Waves A and B are publicly accepted; Wave C awaits immutable publication and external reproduction. Follow the remaining frozen order in the [active plan](https://github.com/ViewCompose/ViewCompose/blob/main/docs/project/plans/ai-verifiable-development-tooling.md): complete Wave C release evidence, then attended public screenshot repair after transactional source-write and rollback acceptance, Compose semantic mapping and Skill guidance, and finally bounded Compose AST conversion. Maintain only explicitly released framework profiles; every mismatch fails closed |
