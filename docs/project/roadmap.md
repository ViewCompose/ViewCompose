...
| AI-verifiable development tooling | Public release `0.6.0` provides version-bound knowledge, one-command onboarding, high-confidence project analysis, and strict offline Figma import. The `0.7.0` release candidate expands the package to 15 CLI/MCP tools and eight Agent Skills, adding non-source-writing screenshot-repair preparation that persists bounded user state plus a separate terminal-attended, atomic, recoverable, conflict-safe single-property apply/rollback host without arbitrary-source or unattended writes | Publish and externally reproduce `0.7.0` through the protected immutable tag, including exact three-client onboarding and attended apply, crash recovery, validation-failure preservation, and explicit rollback evidence. Only after Wave D is publicly accepted may the lower-priority Compose semantic mapping/Skill and final bounded Compose AST converter be reconsidered. Maintain only explicitly released framework profiles; every mismatch fails closed |
