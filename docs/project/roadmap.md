...
| AI-verifiable development tooling | Public release `0.7.0` provides version-bound knowledge, one-command onboarding, high-confidence project analysis, strict offline Figma import, 15 CLI/MCP tools, and eight Agent Skills. Its screenshot-repair path keeps MCP source-write-free and uses a separate terminal-attended, atomic, recoverable, conflict-safe single-property apply/rollback host without arbitrary-source or unattended writes. Protected SLSA publication, three-client onboarding, interrupted-apply recovery, validation-failure preservation, later-edit refusal, and explicit rollback are publicly accepted | Maintain exact released framework profiles and fail closed on every mismatch. Collect adoption and support evidence for the released analyzer, Figma, and attended repair paths. Compose semantic mapping/Skill and a bounded Compose AST converter remain unscheduled at lowest priority until explicit user demand justifies a new activation decision |
