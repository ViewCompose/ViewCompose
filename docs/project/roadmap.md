...
| Collections | LazyColumn/Row/Grid plus Pager, complete list state, sticky headers, content types/spans, prefetch, the neutral compact `LazyItemTable` boundary, and an optional AndroidX Paging `PagingLazyColumn` with explicit placeholders, page-drop handling, load-state projections, lifecycle/mediator fixtures, and a controlled-state Demo | Finish Phase 7 of the active [Paging 3 integration plan](./plans/paging3-integration.md): same-build performance, broader device evidence, Maven/release proof, and archival; Paging remains outside the core contract |
