...
| Collections | LazyColumn/Row/Grid plus Pager, complete list state, sticky headers, content types/spans, prefetch, and an optional AndroidX Paging non-placeholder `PagingLazyColumn` frontend | Continue the active [Paging 3 integration plan](./plans/paging3-integration.md) with the neutral compact item-table hard cut, placeholders, and page drops; Paging remains outside the core contract |
