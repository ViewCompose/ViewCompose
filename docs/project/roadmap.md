...
| Collections | LazyColumn/Row/Grid plus Pager, complete list state, sticky headers, content types/spans, prefetch, the neutral compact `LazyItemTable` boundary, and an optional AndroidX Paging `PagingLazyColumn` with explicit placeholders and page-drop handling | Finish the active [Paging 3 integration plan](./plans/paging3-integration.md) with Phase 3 device/performance closeout, then load-state composition and mediated-data fixtures; Paging remains outside the core contract |
