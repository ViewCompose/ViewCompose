...
36. AI-verifiable development tooling Phase 0 is complete: ADR-0025, five versioned JSON Schemas,
    five reserved capability IDs and Q levels, 17 metric contracts, 14 evaluation cases, 11
    fixture-backed cases, threat controls, and the `qaQuick`-owned `verifyAiToolingContracts` gate
    are accepted. The current generated capability Reference still owns 537 application-facing
    entries, 77 capabilities, 30 artifacts, and one compiled sample per capability. Phase 1 of the
    [active plan](./plans/ai-verifiable-development-tooling.md) now owns the canonical versioned AI
    Knowledge Bundle and compact `llms.txt`; compiler, MCP/CLI/consumer skills, Design IR, and
    conversion or visual-repair pipelines remain unimplemented.
...
| AI-verifiable development tooling | Phase 0 contract, security, version, Q-level, metric, corpus, Schema, and `qaQuick` gate are complete on top of the generated Governance V2 Reference, canonical KDoc, compiled capability samples, diagnostics, and Preview foundation; no supported AI bundle, validator/compiler service, MCP/CLI/consumer skills, Design IR, or converter exists yet | Execute Phase 1 of the [active plan](./plans/ai-verifiable-development-tooling.md): generate and freshness-gate the canonical versioned AI Knowledge Bundle and compact `llms.txt` before validation or protocol work |
