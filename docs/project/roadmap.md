...
| Diagnostics | Correlated render sessions, bounded production failure aggregation, render/layout trees, per-node patches, Locals, recomposition reasons, request-driven real-View highlighting, and finite sampled composition/reconciliation/binding timing | Final inspector, performance, device-matrix, and release closeout remain in the active [diagnostics plan](./plans/diagnostics-correlation-inspection-observability.md) |
