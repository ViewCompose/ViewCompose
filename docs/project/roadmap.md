...
36. AI-verifiable tooling Release `0.3.0` publishes the freshness-gated
    31-artifact/540-symbol Knowledge Bundle and compact `llms.txt`, 13 shared CLI/MCP tools, six
    standard Agent Skills, a reproducible attested offline package, and verified project profiles
    for Codex, Claude Code, and Cursor. Its one current framework profile is selected by exact
    dependency identity and mismatches fail closed. Later bounded phases also provide Android XML
...
| AI-verifiable development tooling | Release `0.3.0` provides a current-version Knowledge Bundle/`llms.txt`, 13 CLI/MCP tools, six Agent Skills, attested offline distribution, Codex/Claude Code/Cursor onboarding, static/compile/Preview/layout evidence, bounded project analysis and XML conversion, and bounded screenshot-to-validated-Kotlin/compare/attended-repair foundations | Follow the frozen order in the [active plan](https://github.com/ViewCompose/ViewCompose/blob/main/docs/project/plans/ai-verifiable-development-tooling.md): public npm one-command onboarding and cross-platform adoption first, then Compose semantic mapping and Skill, bounded Compose AST conversion, enhanced read-only analysis, Figma Design IR import, and finally attended public screenshot repair after transactional source-write and rollback acceptance. Maintain only the current framework profile; every mismatch fails closed |
