...
| AI-verifiable development tooling | Public release `0.6.0` provides the version-bound Knowledge Bundle/`llms.txt`, one-command Agent onboarding, versioned high-confidence project analysis, and 14 CLI/MCP tools plus seven Agent Skills. Its strict offline Figma path performs Design IR import, deterministic Kotlin/PNG generation, and compile/Preview/bounded structure-semantic-geometry-asset comparison without claiming style or pixel parity | Waves A, B, and C are publicly accepted with immutable npm/GitHub provenance and repository-external reproduction. Follow the remaining frozen order in the [active plan](https://github.com/ViewCompose/ViewCompose/blob/main/docs/project/plans/ai-verifiable-development-tooling.md): attended public screenshot repair only after transactional source-write and rollback acceptance, then lower-priority Compose semantic mapping and Skill guidance, and finally bounded Compose AST conversion. Maintain only explicitly released framework profiles; every mismatch fails closed |
