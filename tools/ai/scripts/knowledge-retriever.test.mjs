...
test('loads one integrity-checked immutable knowledge index', async () => {
  ...
  assert.equal(index.manifest.bundleFingerprint, 'ee1765176164201252fe4f3c0b9839a26ee1d87def028255ae2fc435c6594ec1');
  assert.equal(index.artifacts.length, 30);
  assert.equal(index.capabilities.length, 77);
  assert.equal(index.symbols.length, 537);
  assert.equal(index.samples.length, 209);
