...
activity_name="com.viewcompose.SaveableStateTestActivity"
component_name="$package_name/$activity_name"
certification_extra="com.viewcompose.extra.ACTIVITY_ROOT_PROCESS_DEATH_CERTIFICATION"
status_prefix="ACTIVITY_ROOT_PROCESS_DEATH|"
expected_count="41"
...
wait_for_expected_status() {
    ...
    for _ in $(seq 1 80); do
        ...
        if [[ "$status" == "$status_prefix"*"count=$expected_count;pid="* ]]; then
            printf '%s\n' "$status"
            return 0
        fi
        sleep 0.25
    done
    echo "Timed out waiting for Activity-root saveable state. Last status: $status" >&2
    return 1
}
...
initial_status="$(wait_for_expected_status)"
initial_pid="$(
    "$adb_binary" -s "$device_serial" shell pidof "$package_name" |
        tr -d '\r'
)"
...
if ! wait_for_process_exit "$initial_pid"; then
    ...
    if ! wait_for_process_exit "$initial_pid"; then
        echo "The device kept the background application process alive. Real process-death" >&2
        echo "certification requires a device that honors 'am kill' or a userdebug/root target." >&2
        exit 1
    fi
fi

task_lock_output="$(
    "$adb_binary" -s "$device_serial" shell am task lock "$task_id" 2>&1
)" || {
    echo "Unable to restore Android task $task_id: $task_lock_output" >&2
    exit 1
}
restored_status="$(wait_for_expected_status)"
restored_pid="$(
    "$adb_binary" -s "$device_serial" shell pidof "$package_name" |
        tr -d '\r'
)"
...
if [[ "$normalized_initial" != "$normalized_restored" ]]; then
    echo "Activity-root saveable state changed across process death." >&2
    echo "Before: $initial_status" >&2
    echo "After:  $restored_status" >&2
    exit 1
fi

echo "Activity-root process-death restoration passed on $device_serial."
