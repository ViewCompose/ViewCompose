...
translation_source_hash: 2b241b407296ff0752321c2325d59373f7adfa59b4fa3eefcad769414cb93a53
...
| 自适应 Pane 与 Overlay | Navigation 3 Scene 可以选择一个或多个 Entry，并协调 Overlay 与 Transition。1.1.3 和 1.1.4 分别修复嵌套 Overlay 与含 Metadata 的 Popped Entry 动画缺陷。 | ViewCompose 已公开通用的平台无关语义 Scene Value，Host 仍采用最多三个最新 Pane Entry 的固定策略。自适应 Pane 已消费同一转场 Lifecycle Scene，但 Android Host 仍无通用 Overlay 导航 Surface，也没有一份横跨 Overlay 与 Focus 的 Reducer Plan。 | Partially supported | `NavScene.kt`、自适应 Host 测试与 Lifecycle Planner 测试。当前 Partial 的原因是 Overlay 与 Scene Strategy 广度，而不是普通或 Predictive 转场 Lifecycle。 |
