...
translation_source_hash: 10983201568a9e4c242f54681102c35c21f8ba28b98542fff5f794a894da84f7
...
| 目的地生命周期 | 导航 Entry 是 Lifecycle Owner；Navigation 3 Scene 可以呈现多个 Entry。 | `NavSceneEntry` 根据 Presence、Visibility、Interaction、Transition、Pane 与 Layer Role 推导 Scene 和 Entry Cap；Planner 应用 `min(host, scene, entry)`。Settled Pane 可以恢复多个 Entry，而 Android Transition Role 集成仍待完成。 | Partially supported | `NavScene.kt`、`NavLifecyclePlanner.kt` 与穷举式 Planner 测试。迁移代码不能根据 Core Model 推断 Android Transition 行为已经完成。 |
...
| 自适应 Pane 与 Overlay | Navigation 3 Scene 可以选择一个或多个 Entry，并协调 Overlay 与 Transition。1.1.3 和 1.1.4 分别修复嵌套 Overlay 与含 Metadata 的 Popped Entry 动画缺陷。 | ViewCompose 已公开通用的平台无关语义 Scene Value，Host 仍采用最多三个最新 Pane Entry 的固定策略。Android Host 尚未把 Overlay 与 Transition Role 作为一份执行计划消费。 | Partially supported | `NavScene.kt`、Host Runtime 与 Lifecycle Planner 测试。Navigation3 修复改变了对比基线，但不能证明宿主行为等价。 |
...
构造 `NavSceneEntry`；不要在 Scene 旁边重建 Visible 与 Interactive Set。本阶段 Android Host
消费 Settled Pane Projection；完整 Transition Role 集成单独跟踪，不能根据 Core Model 测试
推断宿主已经具备对应行为。
