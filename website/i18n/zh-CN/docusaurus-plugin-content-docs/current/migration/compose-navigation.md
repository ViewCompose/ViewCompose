...
translation_source_hash: 193e0f7b19e3c55c46d9f0272e0eb6d565c9a4d661409915963d211a6e22447e
...
| 返回结果 | Navigation 2 写入上一 Entry 的 `SavedStateHandle`，并在 Lifecycle 安全边界观察；Navigation 3 应用可在其自有 Key 与 State 中建模结果。 | `popBackStack(key, value)` 原子绑定移除与交付。仍存活 Entry 拥有可保存的 FIFO Inbox；`NavResultEffect` 在该 Destination 到达 `RESUMED` 后消费一个类型化值，`peek`/`consume` 则支持显式重试策略。 | Supported | `NavResult.kt`、`NavResultInbox.kt`，以及 Core Reducer、Saved State、Lifecycle Effect 和真机测试。ViewCompose 有意不提供全局结果总线与任意跨栈寻址。 |
