...
translation_source_hash: 4dba1f59cb9d18da1b9efae18a92e0590c77e8dfccc100cf84096fda79b5e499
translation_status: current
---

# Material 3 Android Overlay 集成模块

`viewcompose-overlay-material3-android` 是 ViewCompose Dialog、锚定 Popup、模态 Bottom Sheet、Snackbar
和 Toast 的可选 Android 呈现后端。声明式浮层协议、DSL、队列策略、定位模型及嵌套 Surface
会话位于 `viewcompose-ui-foundation`；本产物负责把这些契约映射为 Android 与 Material 平台窗口。
...
- UI Contract 与 UI Foundation 会被传递暴露，因为它们的 Environment、Builder 和 Overlay
...
本产物通过 Java `ServiceLoader` 注册
`com.viewcompose.host.android.overlay.AndroidOverlayHostFactoryProvider`。标准 ViewCompose
宿主调用 `AndroidOverlayHostDefaults.androidOrNoOp(rootView)`，因此添加依赖即可，无需初始化
Application 或声明 Manifest 组件。

后端独占 `com.viewcompose.overlay.material3.android`；包名明确保留 Material 3 呈现策略，避免
把它误解成通用 Android overlay。
...
UI Foundation 拥有共享 Snackbar/Toast 队列；本模块只提供平台 presenter。因此队列顺序、
...
大多数应用测试应通过 UI Foundation 的 recording host 和队列 snapshot 验证声明式请求。验证
Android Window flag、Material 回调、Popup 几何或系统栏外观时，再使用 Robolectric 或真机测试。

自定义平台后端可以实现 UI Foundation 的 presenter 与 handle 接口，而无需依赖本产物。实现仍需
