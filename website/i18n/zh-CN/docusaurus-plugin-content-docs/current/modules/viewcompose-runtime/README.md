...
translation_source_hash: ae372895249f07143783d85a42dff13bb374e52105a3794012ebee84a37d1972
...
这些修正属于尚未发布的工作树，不会追溯改变已发布制品的行为。嵌套快照固定创建时父方可见的
待提交值，只有后续目标写入参与冲突判断；只读子快照保留相同基线。成功提交，包括空提交，都会
禁止再次进入、写入或提交；子快照不能向已释放或已提交的父方发布。

派生缓存区分可变视图和本地写入。最后一个消费者释放后，上游订阅随之退出。独立读取不保留订阅
但仍保持结果有效，重新观察会在返回前连接依赖。计算失败保留之前的订阅集合，使之后的失效通知
能够触发再次尝试。

通知发生在发布并进入终态之后。每次提交会尝试通知每个受影响的 Observation，直接与派生路径
合计最多一次，重入提交则独立处理。首个回调异常会被重新抛出，后续异常加入 suppressed。此时
值已经提交，因此在收到该错误后重试整个事务可能重复执行应用工作。

2026-09-06 接受的审查基线 `d64710459df73f3b42067767bb2f4f273b9eff33` 中，兄弟快照的
派生值为 10/10，预期为 10/20；嵌套提交出现一个虚假冲突；消费者释放后保留 100 个上游观察者；
首个观察者抛错后，正常观察者收到零次通知。首轮候选源码执行分别得到 10/20、嵌套提交成功、
零个残留观察者，以及完整通知和终态保护。四个已复现运行时缺陷降至零，在这四个场景中减少
100%；正确性结论为“改善”。基线 818 项独立 JVM 测试和首轮候选 835 项测试均通过，新增的
17 项覆盖隔离、空值、嵌套合并、生命周期、回调失败和重入。比较使用 Kotlin 2.2.10、JDK 21、
JVM target 11 和相同源码聚合测试工具，不证明延迟改善、Android 行为或 Gradle 依赖隔离。
下一步通过 Gradle 执行最终模块测试与编译示例，再验证文本、动画和手势集成后关闭执行计划。
...
最终候选增加了一个终态上下文回归：即使在 `enter()` 内调用 `apply()`，全局通知也在已提交的全局
视图中执行。返回后恢复调用方上下文，已提交快照仍禁止继续读写。新增用例先失败，隔离通知上下文后通过。
完整 Gradle 任务 `:viewcompose-runtime:test` 通过 116 项测试，其中 19 项为加固用例，失败和跳过均为零。
相较接受的四个审查缺陷，四个探针均得到修正，正确性结论仍为改进；扩大覆盖不等于性能测量。
后续保留模块回归，并单独使用真机和高扇出基准评估性能。
