...
translation_source_hash: 164eb2dab64bbe4fe34435a759c3397269be0e5ed9bf064c8f77f064365c5f03
...
预算模型把合法发布历史增长与真正回归分开。当前上限为：非 API 产物 46.7 MiB、
...
非 API 上限从 41 MiB 演进到 46.7 MiB 的每一步，都先通过成对构建把增长归因到长期双语契约，
并通过表示审查移除可避免的重复。下方把已完成测量收敛为紧凑记录，不再在有效契约中重复每个
执行阶段。任何阈值调整都必须提供同语料的绝对值和归一化结果、读者或发布价值、结论、限制以及
下一项停止条件。

达到 46.7 MiB 边界后，失败分支必须先合并已完成证据或改变站点表示。不得仅为回收预算而删除当前
...
- **2026-08-25，Paging Phase 2 发布 Slice：**在同一份本地 macOS Lockfile 上，Phase 0 候选与
  Phase 2 候选的非 API 产物分别为 48,751,657 和 48,942,128 字节。增加的 190,471 字节
  （0.391%）来自双语当前模块手册，以及目录、指南与路线图契约；API 产物继续由独立预算约束。
  候选超过原 46.5 MiB 上限 183,344 字节，因此采用最小的 0.1 MiB 步进，把上限调整为
  46.7 MiB，测得余量 26,371 字节。结果为**混合**：440 个站点页面现在用两种语言公开可选
  Paging 前端及其生命周期、标识与非占位限制，但非 API 存储量有所增加。语言、翻译、路由、
  无障碍和全部预算均通过。本次同主机对照不衡量部署传输、CDN 压缩或运行时网络行为。Phase 3
  必须先收敛表示方式，才能再次提高非 API 阈值，并继续让生成 API 历史服从独立的单树预算。

这里只展开最新的同语料决策；Git 历史保留 2026-08-06 至 Phase 0 检查点被后续结论取代的
检查点明细。那些测量建立了不可变 API 历史重建、双语搜索与无障碍门禁、计划和证据分段，
并要求非 API 上限从 41 MiB 演进到 46.7 MiB 前先做重复内容收敛。历史中既有长期契约增长
