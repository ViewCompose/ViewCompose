...
translation_source_hash: 3225f372d484a87dfd0f98f806af5694b22d297c41780e32dd7eb2147f21163f
...
上限从 41 MiB 调整到 46.9 MiB 前均经过成对归因和内容收敛。当前棘轮要求通过结构优化恢复容量，
不能再次提高阈值；只能删除重复部署表示，不能删除当前契约或有效发布历史。已有受测试保护的
Transform 会移除无用 Locale 副本、仅机器读取的治理/翻译 Front Matter、不可变手册 Hydration 和
生成缩进，同时保持路由与可读内容不变。历史同语料测量及限制保留在下方源码中，不再重复进入公共
运维契约。
