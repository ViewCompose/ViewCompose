...
translation_source_hash: 542acba88f1b37a8e06ae2cbe52bbb0638f3e232a41f615bc518c4f8a4f9724f
...
预算模型把合法发布历史增长与真正回归分开。当前上限为：非 API 产物 46 MiB、
...
非 API 上限从 41 MiB 演进到 46 MiB 的每一步，都先通过成对构建把增长归因到长期双语契约，
并通过表示审查移除可避免的重复。下方把已完成测量收敛为紧凑记录，不再在有效契约中重复每个
执行阶段。任何阈值调整都必须提供同语料的绝对值和归一化结果、读者或发布价值、结论、限制以及
下一项停止条件。

达到 46 MiB 边界后，失败分支必须先合并已完成证据或改变站点表示。不得仅为回收预算而删除当前
