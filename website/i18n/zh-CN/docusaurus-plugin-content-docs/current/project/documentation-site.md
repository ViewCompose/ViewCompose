...
translation_source_hash: 1eb06983dbdb488ebfca79dbeb875bcf71a3b410c7713272177992080dbd0091
...
- **2026-08-25，Paging Phase 0 契约：**在同一份本地 macOS Lockfile 和包含 100 个不可变、
  5 个未发布 Current API 树的完整语料上，main、初始候选和含证据的紧凑候选分别为
  48,752,081、48,821,194 和 48,751,657 个非 API 字节。初始增长 69,113 字节（0.142%），
  超过 46.5 MiB 上限 62,410 字节；收敛重复计划规则和被取代的站点台账叙述后减少
  69,537 字节（0.142%），没有删除冻结契约或提高阈值。表示方式结论为**改进**；438 个页面、
  两种语言、搜索、路由、无障碍和预算均通过。对照只覆盖一台本地主机，不衡量部署或网络行为。
  CI 是下一道门；后续阶段继续让每项契约只有一个归属段落。

这里只展开最新的同语料决策；Git 历史保留 2026-08-06 至 2026-08-24 被后续结论取代的
检查点明细。那些测量建立了不可变 API 历史重建、双语搜索与无障碍门禁、计划和证据分段，
并要求非 API 上限从 41 MiB 演进到 46.5 MiB 前先做重复内容收敛。历史中既有长期契约增长
