...
translation_source_hash: bdc1f3dcc70d5b349feb797313e642b81c37de86d1e6b4ae2c9e6e29c1ebdb65
...
| AI 可验证开发工具 | 公开 Release `0.6.0` 已提供版本绑定 Knowledge、单命令接入、高置信度 Project 分析与严格离线 Figma Import。`0.7.0` Release Candidate 把 Package 扩展到 15 个 CLI/MCP 工具和 8 个 Agent Skill，新增不会写 Project 源码、但会持久化有界用户状态的 Screenshot Repair Preparation，以及独立的 Terminal 人工授权、Atomic、可恢复、Conflict-safe 单 Property Apply/Rollback Host，不提供任意源码或 Unattended Write | 通过受保护的不可变 Tag 发布并在 Repository 外复现 `0.7.0`，覆盖 3 个客户端的精确接入，以及人工授权 Apply、Crash Recovery、Validation-failure Preservation 与显式 Rollback 证据。只有 Wave D 通过公开验收后，才重新考虑低优先级 Compose 语义 Mapping/Skill 与最终有界 Compose AST Converter。只维护明确发布的 Framework Profile；任何版本不匹配都 Fail closed |
