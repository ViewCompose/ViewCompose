...
translation_source_hash: ba73002b5aa4aa7f27873b10134c5d6580a8d709b06a03c5ddd669d21f6333af
...
| AI 可验证开发工具 | 公开 Release `0.7.0` 已提供版本绑定 Knowledge、单命令接入、高置信度 Project 分析、严格离线 Figma Import、15 个 CLI/MCP 工具和 8 个 Agent Skill。Screenshot Repair 路径不向 MCP 授予源码写入权限，而是使用独立的 Terminal 人工授权、Atomic、可恢复、Conflict-safe 单 Property Apply/Rollback Host，不提供任意源码或 Unattended Write。受保护 SLSA 发布、3 个客户端接入、Interrupted-apply Recovery、Validation-failure Preservation、后续编辑保护与显式 Rollback 均已通过公开验收 | 只维护精确发布的 Framework Profile，任何版本不匹配都 Fail closed；继续收集已发布 Analyzer、Figma 与人工 Repair 路径的接入和支持证据。Compose 语义 Mapping/Skill 与有界 Compose AST Converter 保持未排期和最低优先级，直到明确用户需求支持新的激活决策 |
