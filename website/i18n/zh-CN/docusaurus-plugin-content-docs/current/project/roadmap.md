...
translation_source_hash: fbf8dded9da0f3941bab119cf87384a759d0d483d15ae2263ffa2b59edd744db
...
| AI 可验证开发工具 | 公开 Release `0.6.0` 已提供版本绑定的 Knowledge Bundle/`llms.txt`、单命令 Agent 接入、版本化高置信度 Project 分析，以及 14 个 CLI/MCP 工具与 7 个 Agent Skill。严格离线 Figma 路径支持 Design IR Import、确定性 Kotlin/PNG 生成，以及编译、Preview 和有界结构/语义/几何/Asset 比较，但不声称 Style 或 Pixel Parity | Wave A、Wave B 与 Wave C 均已通过公开验收，并具有不可变 npm/GitHub Provenance 与 Repository 外复现。按[有效计划](https://github.com/ViewCompose/ViewCompose/blob/main/docs/project/plans/ai-verifiable-development-tooling.md)剩余冻结顺序推进：只有事务化源码写入与回滚门禁通过后才公开人工授权截图修复；低优先级的 Compose 语义映射与 Skill 指导随后推进，有界 Compose AST 转换放在最后。只维护明确发布的框架 Profile，任何版本不匹配都拒绝接入并保持原状 |
